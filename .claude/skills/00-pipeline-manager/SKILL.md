...
only the owner can resolve** — not merely "big," "a lot of work," or "touches production code" —
and only *then* does it stop and report. A gap closeable by any pipeline stage's own automated
work (either `02-research-*` tier, `03` architecture, `04` requirements, `05` decomposition, `06`
spec, `07` planning) is never itself a reason to stop; the manager runs that stage next instead of
asking. The genuine stopping conditions are: **G3** package authorization, **G4** release GO, a
**Vision-level** tension only `01-vision`'s own owner-facing judgment can settle, a ripe
**`NEEDS-USER`** backlog entry, an unadjudicated Critical/High review finding, or the
backlog/next-step queue being truly empty (nothing left to advance). See `step` mode below for the
old one-stage-at-a-time behavior when that's what's actually wanted.
...
1. **Position block** (rewritten every run):
   ...
   - **Open gates:** <owner decisions pending: authorizations, GO calls, unadjudicated findings — or "none">
   ```
...
2. **Run log** (append-only, newest last — never rewrite or delete old rows):
   ...
   **Archiving:** once the live run log passes roughly 200 rows, move the oldest rows to
   `docs/pipeline/pipeline-journal-archive.md` (verbatim, same table format, append-only there too)
   and leave a one-line pointer in their place. Never delete a row — only relocate it. This is a
   doc-scope refactoring package (`08-refactoring`), not something to do ad hoc mid-run.
...
filed by the `00-intake` skill. One table, `BL-####` IDs, append entries / update statuses in
...
**Archiving:** once a row flips `DONE`/`REJECTED`, it moves to `docs/pipeline/backlog-archive.md`
(verbatim, same table format, in original ID order) at the next triage sweep, rather than staying
live indefinitely. Never delete a row — only relocate it. This keeps the live backlog focused on
...
| `status` | Read-only: reconcile in-memory, print the stage survey + backlog summary + recommendation. No writes unless drift was found and the owner confirms syncing it. |
...
`docs/master/INDEX.md` · `docs/architecture/INDEX.md` §1 (GDS ladder) · `docs/requirements/` +
...
2. **Disposition every `NEW` entry.** For each, decide and record: fold into the upcoming step
   ...
   explicit revisit trigger (`DEFERRED`), require an owner decision (`NEEDS-USER`), or reject with
   a reason (`REJECTED`). Severity honesty applies: a Critical/High finding may not be quietly
   `DEFERRED` — that disposition needs the owner's explicit agreement.
...
4. **Batch the owner questions — but only within the same tier.** Collect all ripe `NEEDS-USER`
   ...
   *highest* tier present. Before batching, check each lower-tier ripe entry for **tier
   ...
   moot this question? If yes, hold the lower-tier entry back this run — do not ask it alongside
   the higher-tier one — and re-derive whether it's still live once the higher-tier answer lands.
   If a lower-tier entry is genuinely independent of every open higher-tier question (touches a
   different, unrelated part of the tree), it may still batch into the same `AskUserQuestion` call
   rather than costing the owner a second round-trip.
...
- **Don't ask a lower-tier question the tree hasn't earned yet.** If a Vision-level tension is
  still open (e.g. an unresolved `OQ-##` in `00-vision.md`/`MSTR-001` bearing on the same feature),
  route there first — a downstream Architecture/Requirements-level question that depends on the
  answer is premature, not merely optional.
- **Re-check, don't re-ask, after a higher tier resolves.** Once a higher-tier answer lands, the
  ...
  question drafted before the higher-tier context existed.
- **Independent threads don't wait on each other.** Tier precedence governs questions that
  actually depend on one another — it is not a rule that only one open thread in the whole tree
  may be asked about at a time.
- **Record the hold, not just the ask.** When a lower-tier entry is held back for this reason,
  ...
  ID/question>") and in the run's journal row.
...
outranks it, that's the default. If several steps are genuinely parallel, pick one and name the
...
Before invoking anything, stop and ask the owner (via `AskUserQuestion`) if the step requires:

- **G3 authorization** — the step would implement a package that is neither (a) already scheduled
  by the current, owner-approved release plan in the shape the plan describes, nor (b) carrying
  its own explicit owner go-ahead on record. Release-plan coverage satisfies G3 automatically —
  cite the matching release-plan section/row rather than asking again. A package not on the
  release plan, or diverging materially from what it describes, still requires a fresh per-package
  go-ahead before any `08-code-implementation`/`08-content-authoring`/`08-refactoring` run;
...
- **a ripe `NEEDS-USER` backlog entry** — the decision the entry is waiting on is needed now, and
  ...
  to the owner this run;
- **spending judgment the owner reserved** — anything a stage skill's own rules say needs the owner.
...
that terminates iteration before the backlog/next-step queue is empty. Record the owner's answers
...
the durable record of every step taken — the chat-facing report is a separate, one-time thing
composed only once the loop actually stops (genuine gate, or the queue is empty). Loop back to
Step 1 instead of narrating progress mid-run.
...
executed only by `08-refactoring`) is deliberate housekeeping, not a default: the manager schedules
it only under the conditions below, and never lets it ride along with feature or fix work.
...
- three or more open backlog entries trace to the same structural cause (the manager names them
  on the new entry);
- a stage-08 run's Outstanding Issues repeatedly cite the same friction;
...
1. the `refactor` entry is dispositioned `SCHEDULED` with the owner aware of it;
...
1. the package is `READY` (dependencies `VERIFIED`) **and** clears G3;
...
3. the tree is green: the G5 gates pass as-found (never schedule a refactor onto a red tree);
4. no release bucket is mid-close (between a `10-integration-review` and its `11` GO call) whose
   reviewed set overlaps the refactor's files.
...
- **One skill invocation per internal step, always.**
- **Iterate mode stops only for a genuine gate, never for scope.** A step being large,
  ...
  truly empty queue stop the loop.
- **Don't narrate mid-loop.**
- **Higher tier first.**
- **No finding left in chat.** If a stage skill said it, the backlog holds it.
- **Never bypass a gate**, even in `run <skill>` override mode.
- **Never perform stage work inline.**
...
- **The journal and backlog are honest or they are useless.**
...
- [ ] Every `NEW` backlog entry left this run with a recorded disposition, and no Critical/High
      entry was deferred without the owner's explicit agreement.
- [ ] If more than one tier had a ripe open question, only the highest tier (plus genuinely
      tier-independent ones) was put to the owner this run.
...
- [ ] Every gate touched by any step was stopped at and asked about — none assumed, and in
      iterate mode a gate hit anywhere in the loop actually ended the loop.
- [ ] In iterate mode, the loop did not stop early for a step that merely looked large or
      unfamiliar.
- [ ] Every run-log row, Position-block rewrite, and backlog update was written and committed
      before the next step was chosen, and matches what actually happened.
- [ ] In iterate mode, no chat-facing report was produced between internal steps.
...
2. **Recommendations** — open gates awaiting the owner, `NEEDS-USER` backlog entries and the exact
...
3. **Next step** — what the next invocation will execute (skill + target + any backlog entries
   riding along + why), or the exact decision needed if the pipeline is gated on the owner.

**Iterate mode (no args): produce this same three-part summary exactly once, when the loop
actually stops** — not after every internal step.
