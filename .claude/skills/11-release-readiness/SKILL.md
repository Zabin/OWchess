...
description: Make the evidence-based go/no-go assessment for one release bucket and, on GO, update the engineering baseline — produce a Release Assessment under docs/reviews/ (scope delivered vs. planned, verification and integration evidence, known deviations, residual risks, explicit GO/NO-GO), and on GO flip the baseline records (release-plan bucket state, CLAUDE.md status line, affected INDEX files) so the whole tree agrees the release shipped. Use when asked "are we ready to release," "run the release readiness review," "close out the baseline/MVP bucket," or after 10-integration-review comes back clean for a release's scope. The GO/NO-GO recommendation is advisory — the owner makes the actual release decision (G4); this skill never writes code, never edits packages/specs/requirements, and performs no deployment. Do not use it to review package integration (10) or to verify packages (09).
---
...
nothing has shipped yet): assembles the evidence that the bucket's promised scope was delivered,
verified, and integration-reviewed; states an explicit **GO / NO-GO** recommendation with the
reasoning; and — on the owner's GO — updates the engineering baseline so every tracker agrees the
release shipped. The final stage; its output is the record the *next* increment plans against.
...
in-pass) · deploy anything · declare GO on the owner's behalf (G4 — the recommendation is
advisory; the owner makes the call before any baseline flip).
...
and Content Review reports for this scope (`docs/reviews/`), the RTM.
...
4. **Write the Release Assessment** → `docs/reviews/release-assessment-<release>.md`: **Release**
   ...
   VR(s) → integration coverage → delivered/deviated) · **Evidence** (build/start + test-suite
   ...
   NO-GO).
5. **On the owner's explicit GO — update the baseline** (the only mutating step, trackers only):
   flip the bucket's state in `01-release-plan.md`; update `CLAUDE.md`'s status/version line (once
   it exists); update affected `INDEX.md` files; date-stamp the baseline in the assessment. Commit
...
- [ ] The GO/NO-GO reasoning is stated, and no baseline record flipped without the owner's
      explicit GO.
- [ ] After a GO flip: release plan, Master Build Plan, and CLAUDE.md agree — no tracker telling
      the old story.
...
1. **What changed** — the Release Assessment written (path + GO/NO-GO), and any baseline records
   flipped (only after an explicit owner GO).
2. **Recommendations** — on NO-GO, the blocking items each with its owning skill; on GO, the
   residual risks the owner is accepting and any deferred scope now owed to a future bucket.
