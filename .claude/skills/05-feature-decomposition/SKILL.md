...
**ID conventions:** catalog rows are `FEAT-####` (4-digit, gapped) — deliberately **not**
`FS-###`, which stage 06's full specification documents own. Epics are `EP-####`.
...
turn/session lifecycle (deployment, alternation, win check) · the F2T2E chain mechanics · the
asset roster & mission sets · effect resolution (the Five D's) · orbital regime & the `Propagator`
boundary · fog-of-war/belief-state · server-authoritative transport (WebSocket) · board/UI
presentation — let the FR groupings lead, don't force this list). Maximize cohesion, minimize
coupling; align Feature boundaries with module boundaries wherever the requirements support it,
and justify any straddle explicitly.

Every Feature carries the fixed field set: Feature ID (`FEAT-####`) · Title · Purpose ·
...
Group Features into Epics (`EP-####`): ID · Title · Purpose · Features Included (complete list) ·
...
requirements, decomposed, planned) rather than by already existing in a shipped game. Document
...
Foundational · Optional · Deferred. A natural MVP floor for this project is a single-mission-set,
minimal-asset-roster, two-player game that proves the F2T2E loop and fog-of-war end to end —
but the actual bucket assignment is this step's own decision, not assumed here.
...
- **Catalog rows are not Feature Specifications.** A `FEAT-####` row is a planning-grain summary
  that stage 06 expands into a full `FS-###` document under `docs/features/` — don't let a
