...
| ~~CR-03~~ | ~~Per-regime-pair maneuver fuel-analog budget and transfer-time-in-turns table~~ | **Resolved 2026-08-22 — see FS-104's Maneuver Cost Table.** R-201 grounded the relative Δv/time-of-flight shape (two-body Hohmann transfers + plane-class changes); FS-104 (`docs/features/FS-104-orbital-mechanics-propagator.md`) adopted it into a concrete altitude-component/plane-component/combined-maneuver formula, satisfying FR-5300/5400's deferred "exact per-asset budget: `06-feature-specification`" clause. No longer a candidate. |
