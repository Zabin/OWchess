...
- **FR-2300** — **Belief-state staleness/decay.** The system shall degrade a `BeliefStateEntry`'s confidence if not refreshed by continued tasking over turns.
  - *Rationale:* SOR §7.7; FR-3004. *Priority:* Must. *Acceptance Criteria:* Given a belief entry last updated at turn N, when N+k turns elapse without re-tasking (k: TBD — see Candidate Requirements below), then the entry's precision is reduced by one level or flagged stale. *Dependencies:* FR-2100. *Verification:* Test. *Source:* SOR §7.7; GDS-07 §BeliefStateEntry. *Notes:* the exact decay rate `k` is **not decided in this pass** — see Candidate Requirements; this leaf establishes the mechanism's existence, not its numeric rate.
...
- **FR-5100** — **Real propagation.** The system shall propagate every asset's true position using a Kepler+J2-minimum model, regardless of what precision is exposed to either player.
  - *Rationale:* FR-5001; SOR §7.6. *Priority:* Must. *Acceptance Criteria:* Given any asset, when a turn advances, then `Propagator.advance` updates its true orbital elements per Kepler+J2 math, independent of either player's belief-state. *Dependencies:* none. *Verification:* Test (deterministic given fixed elements) + Analysis (compare against R-201/202 once authored). *Source:* SOR FR-5001; GDS-03 §`Propagator`; R-203.
...
- **FR-7300** — **Disconnect/reconnect handling.** The system shall allow a player to reconnect to an in-progress session without corrupting game state (exact grace-period duration: Candidate Requirement below).
  - *Rationale:* FR-6003. *Priority:* Must. *Acceptance Criteria:* Given a player's connection drops mid-session, when that player reconnects with the same `sessionId`, then their client receives the current authoritative state and can resume acting on their next legal turn. *Dependencies:* FR-1220. *Verification:* Test. *Source:* SOR FR-6003. *Notes:* the exact grace-period/forfeit-on-timeout policy is **not decided in this pass** — see Candidate Requirements and the Strategic Assumptions Register's disconnect risk item.
...
|---|---|---|
| CR-01 | Belief-state decay rate (turns before a precision downgrade) | No source document fixes a rate; FR-2300 establishes the mechanism only. Owner: `06-feature-specification`, per its own "feature-level numeric refinement" scope. |
| CR-02 | Disconnect/reconnect grace-period duration and forfeit-on-timeout policy | SOR explicitly defers this (FR-6003's own text); no numeric value or policy shape is fixed anywhere upstream. Owner: `06-feature-specification`, flagged as a real design decision (per-turn time limit vs. auto-pass vs. forfeit — Strategic Assumptions Register risk table already names this). |
| CR-03 | Per-regime-pair maneuver fuel-analog budget and transfer-time-in-turns table | FR-5300/5400 establish the mechanism; the actual numbers depend on R-201/202 (not yet authored) for real Kepler+J2 delta-v figures. Owner: `02-research-orbital-and-tooling` (grounding) then `06-feature-specification` (adoption). |
