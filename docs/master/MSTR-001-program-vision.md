...
- **Document ID:** MSTR-001 · **Version:** 0.2 · **Status:** 🟢 Gate-confirmed — the owner has
  ...
  downstream stage as originally scoped.
- **Date:** 2026-08-21 · **Owned by:** `01-vision` skill
...
gameplay convenience. Orbital mechanics are propagated internally with real Kepler+J2-minimum
math, but presented to players as discrete, chess-legible regimes/slots (e.g., LEO/MEO/GEO-analog
bands) — simple for gameplay, real math underneath, mirroring the ZabOW `ORBITAL COMMAND`
reference's own LEO/MEO/GEO radial-band presentation almost exactly (§4).
...
| C4 | Orbital mechanics are **hybrid-fidelity**: real Kepler+J2-minimum propagation internally, discrete named-regime/slot presentation externally, behind a `Propagator`-equivalent interface boundary that isolates the implementation so a future higher-fidelity swap (or shared code with `ZabSpaceExercise`) is a swap, not a rewrite. | SOR §7.6, FR-5001–5005, NFR-5003. |
